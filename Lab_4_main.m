...
title('S(t)'); xlabel('Time (s)'); ylabel('Amp');
...
fft_Mx = fft(M_x);
P2_Mx = abs(fft_Mx/L); 
P1_Mx = P2_Mx(1:floor(L/2)+1); 
P1_Mx(2:end-1) = 2*P1_Mx(2:end-1);
fft_My = fft(M_y);
P2_My = abs(fft_My/L); 
P1_My = P2_My(1:floor(L/2)+1); 
P1_My(2:end-1) = 2 * P1_My(2:end-1);
plot(f_axis, P1_Mx, 'b', f_axis, P1_My, 'r'); xlim([0 300]);
title('Mixed Signal FFT'); xlabel('Freq (Hz)');
legend('M_x FFT', 'M_y FFT');
...
fprintf('實驗振幅 R: %.4f (Rel Err: %.2f%%)\n', R_exp_single, abs(R_exp_single-R)/R*100);
fprintf('實驗相位 phi: %.2f deg (Abs Err: %.4f deg)\n', ...
    phi_exp_single*180/pi, abs(phi_exp_single-phi)*180/pi);
fprintf('==============================================\n');
...
ax1 = subplot(2, 1, 1); title('R(t)'); grid on; hold on;
yline(R, 'k--', 'LineWidth', 1.5); 

ax2 = subplot(2, 1, 2); title('phi(t)'); grid on; hold on;
